...
- Resource history stores `cpu_power_w` (measured CPU package power) and each GPU's `power_w`. The 1h/24h bucket response also includes power sample counts; the UI calculates "other power" only when the total, CPU, 3090, and 4090 readings cover every sample in the bucket.
