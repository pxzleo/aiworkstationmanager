...
| GET | `/api/v1/history` | Resource history using `window` and an optional timezone-aware ISO 8601 `end` query parameter |
| GET | `/api/v1/host-services` | Host-service summary |
| GET | `/api/v1/power-model` | Total-system-power estimate parameters, calibration record, and the current measured/estimated split |
| PUT | `/api/v1/power-model/electricity-rate` | Save the electricity rate per kWh; requires CSRF |
| POST | `/api/v1/power-model/calibration` | Calibrate total system power from a power-meter reading; requires CSRF |
| DELETE | `/api/v1/power-model/calibration` | Clear the calibration and return to the default estimate parameters; requires CSRF |
...
| GET | `/api/v1/automatic-tasks/models` | Read available models live from the local OpenCode CLI as an authenticated user |
| GET | `/api/v1/automatic-tasks/execution` | Read the shared daily schedule and current process OpenCode launch status as an authenticated user |
| PUT | `/api/v1/automatic-tasks/execution` | Save `enabled`, local `time_local` (`HH:MM`), and an existing absolute `working_directory` with CSRF; audit the change |
| POST | `/api/v1/automatic-tasks/execution/start` | Launch the pending queue immediately with CSRF and audit the launch request; reject an empty queue, an existing executor, or unavailable OpenCode |
| POST | `/api/v1/automatic-tasks/execution/start-local` | Let only a local OpenCode plugin ask AXIS to start the whole queue without browser cookies |
| POST | `/api/v1/automatic-tasks` | Create a task with CSRF; the body contains `content` and optional `model` |
...
| POST | `/api/v1/automatic-tasks/{id}/reset` | Requeue a running, succeeded, or failed task with CSRF; clear its result and ownership, preserve its cumulative attempt count, and invalidate its previous execution token |
| POST | `/api/v1/automatic-tasks/claim` | Atomically claim the first pending task in the saved queue order for the current local OpenCode `session_id`; AXIS-launched single-task sessions also send `expected_task_id`, and a changed queue is rejected |
...
The Execution tab saves one parent directory and local daily time for the whole queue; scheduling is disabled by default, and the manual start button works independently of the switch. AXIS creates a separate child directory and new local OpenCode CLI session for each task in queue order. Each session uses the `axis-automatic-tasks` Skill to claim only its specified task, writes the result, and exits before AXIS starts the next task. The triggered local date is stored to avoid a second daily launch after a manager restart. Enabling a schedule after today's time has passed starts it the next day. The page displays launch status and errors; individual task states remain in the queue database.

The UI accepts task instructions and an optional execution model; AXIS generates a compact title from the first meaningful sentence. It loads every page using immutable creation order, then displays tasks in execution order. Pending tasks can be moved up or down; the server atomically compares the order originally read by the page and the complete pending set, rejecting concurrent changes explicitly. Running, succeeded, and failed tasks can be requeued at the pending tail. Requeue clears their result and execution ownership, preserves the cumulative attempt count, and increments it on the next claim. A running task's old token becomes invalid immediately, but externally started work is not cancelled. After a claim, the Skill does not stop to ask the user questions or wait for human intervention. Non-critical choices that do not expand authorization, change the task goal or key artifact, or cause irreversible effects use existing project conventions, defaults, and the minimal viable approach, and are recorded in the result summary. Information available from the workspace, existing configuration, or safe read-only checks must be resolved autonomously. A task may fail only when indispensable credentials, assets, or external conditions are missing after safe bounded retries; its instructions conflict internally; a key goal or artifact specification cannot be resolved from conventions, configuration, and read-only checks; objective acceptance criteria remain unmet after reasonable attempts; or new authorization for a high-impact action is required. Unattended execution does not expand authorization for deletion, publication, payment, or external messages. The `axis-automatic-tasks` Skill asks AXIS to start the queue from a normal session. In an AXIS-launched single-task session it uses claim, heartbeat, and finish tools only for the specified task. After a claim, the plugin renews the 30-minute lease every minute in the background, including while a long-running tool is active, a single response has failed, or the session is idle waiting for AXIS video generation and its callback. `session.idle`, `status=idle`, and a single `session.error` do not end an automatic-task lease. Network and transient server errors keep retrying; definitive task-not-found, expired-lease, or owner-mismatch responses stop background renewal. An expired claim is recovered by the next claim transaction and its old token cannot write a late result. AXIS starts the next item in a new child directory and session only after the current session records its result and exits; a failed item records completed checks and its cause before the queue continues. Running items cannot be edited or deleted; a second OpenCode session cannot claim work concurrently. Run `integrations/opencode/Install-AxisAutomaticTasks.ps1`, restart OpenCode, and ask it to start automatic tasks.
...
When a job containing a local source video is submitted, AXIS reads its absolute path from the persisted workflow and stores its size and SHA-256 identity. After success it atomically publishes the same content to `file_service_root/输出/<original filename>`. The original remains unchanged. An identical existing copy is reused idempotently; changed source content, a same-name file with different content, a non-absolute or escaping path, or any copy failure fails the video job explicitly without overwriting the existing file. Restart recovery repeats the same identity-checked publication step.

For clothing-change and occlusion-removal tasks, both video Skills use the no-verification fast path by default: they do not create samples or run final-frame, media-stream, or audio-hash checks. They perform only the verification scope explicitly requested when the user asks to verify, validate, review the final video, or create a sample. Default no-verification still retains source parameter probing and isolated source visual analysis needed to write the prompt, plus safe segmentation, formal generation, assembly, format conversion, and audio restoration. `finish_video.py` only finishes the media by default. New calls use `--verify --report <name>` when verification is explicitly requested for a clothing-change task or when another Ref2V task follows its normal validation path. To avoid breaking an already-running old Skill invocation, `--report` alone retains the legacy explicit-verification behavior. Other video tasks retain their existing validation rules.
...
- `/api/v1/history` uses a minute-formatted `window`, defaults to `15m`, and accepts `1m..44700m`. Optional timezone-aware ISO 8601 `end` selects the upper bound; the query returns samples within `[end-window, end)`. `15m` returns raw samples, `1h` uses 15-second buckets, a calendar day uses 60-second buckets, a calendar week uses 15-minute buckets, and a calendar month uses hourly buckets. In addition to `samples`, the response still includes `bucket_seconds`, `retention_minutes`, `stored_sample_count`, `stored_since`, and `stored_until` so clients can determine historical coverage; the current resource-monitor UI does not display this metadata.
...
- `/api/v1/power-model/calibration` takes `{"point":"idle","wall_w":340}`. `point` accepts only `idle` or `load`, and `wall_w` is the wall power reported by a power meter, ranging over `0..5000` exclusive of 0. The server records the measured power at the moment of submission as that calibration point; with both points present it solves for the fixed baseline and the PSU efficiency together, and with one point the PSU efficiency keeps its configured default. Parameters that solve outside plausible bounds return 422.
- `/api/v1/power-model` returns `model` (the active estimate parameters and their `source`), `calibration` (the raw calibration record, `null` when uncalibrated), `electricity_rate_yuan_per_kwh` (the saved rate, defaulting to 0.5 CNY), and `current` (the present `measured_w`, `estimated_w`, `total_w`, and `accounted_dc_w`). `PUT /api/v1/power-model/electricity-rate` accepts the `yuan_per_kwh` field, for example `{"yuan_per_kwh":0.5}`; the rate must be finite and nonnegative and is stored in the database.
- The snapshot's `host.power` splits power into three bases: `measured_w` covers only real sensor readings (GPU board power and CPU package power), `estimated_w` covers the motherboard, memory, drives, fans, CPU VRM losses, and PSU conversion losses, and `total_w` is their sum. `estimate_breakdown` itemizes the estimate, and each entry in `sensors` carries a `role` and a `method` (`energy_delta` for an interval average derived from the energy accumulation counter, `instant` for a point reading). When no sensor can be read, all three fields are `null` rather than a fabricated 0.
- Resource history stores `cpu_power_w` (measured CPU package power) and each GPU's `power_w`. Bucketed responses also include power sample counts; the UI calculates "other power" only when the total, CPU, 3090, and 4090 readings cover every sample in the bucket.
...
WM_POWER_ESTIMATE_ENABLED
WM_POWER_CPU_VRM_EFFICIENCY
WM_POWER_BASELINE_W
WM_POWER_PSU_EFFICIENCY
WM_POWER_PSU_RATED_W
```
...
The default database is `data/workstation-manager.db`. The current schema is 37 and migrates automatically at startup. Schema 19 adds the unique scene `is_default` marker. Schema 20 adds the separate scene `detailed_description` field. Schema 21 adds the authoritative operation `total_steps` count. Schema 22 adds explicit WSL `portproxy` configuration to registered services. Schema 23 adds the legacy scene-purpose fields, the persistent `video_jobs` state machine, and RTX 4090 `resource_leases`. Schema 24 removes the video-job callback authorization field. Schema 25 migrates the legacy `video_gen` purpose into the unique `is_default_generation` checkbox and stores each video job's generation and original scenes. Schema 26 adds explicit video batch IDs, segment indices, and segment counts. Schema 27 persists each segment's workflow-derived video specification so job listings do not repeatedly parse full workflows. Schema 28 adds the source-video title or prompt summary to that specification and backfills existing jobs. Schema 29 persists the relative output path successfully published to the shared file service. Schema 30 adds the automatic-task queue, execution-session ownership, and result status. Schema 31 adds a persistent pending-task order. Schema 32 repairs null queue positions written by an old process after the schema 31 migration by appending them to the existing queue in creation order. Schema 33 adds the total-system-power column to resource samples. Schema 34 splits total system power into measured and estimated columns and adds the `power_calibration` table that stores wall-meter calibration results. Schema 35 adds CPU package power to resource history for the synchronized power-breakdown curves. Schema 36 adds a persistent electricity-rate table with a default of 0.5 CNY/kWh. Schema 37 adds the shared automatic-task execution settings and last daily trigger date. Existing details and default-generation values remain unchanged when an older client omits those fields. Only one manager instance may use a database at a time, preventing duplicate script execution.

The service control plane stores desired and observed states separately. Scenes, the overview, and GPU service summaries use only observed state. SQLite is updated only when the state or error changes, so successful five-second checks do not write continuously. Neither scheduled resource sampling nor health monitoring runs service scripts; explicit deep checks, startup reconciliation without a default scene, and failed-action reconciliation invoke `status`. Resource sampling writes CPU, memory, and per-GPU load, VRAM, temperature, power, and graphics-clock metrics to SQLite and retains the latest 90 days by default; the in-memory queue remains limited to the latest 15 minutes.
