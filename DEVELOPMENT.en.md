...
The Execution tab saves one working directory and local daily time for the whole queue; scheduling is disabled by default, and the manual start button works independently of the switch. AXIS starts a local OpenCode CLI session instructed to use the existing `axis-automatic-tasks` Skill. The triggered local date is stored to avoid a second daily launch after a manager restart. Enabling a schedule after today's time has passed starts it the next day. The page displays launch status and errors; individual task states remain in the queue database.
