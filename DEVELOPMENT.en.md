...
- `/api/v1/power-model` returns `model` (the active estimate parameters and their `source`), `calibration` (the raw calibration record, `null` when uncalibrated), and `current` (the present `measured_w`, `estimated_w`, `total_w`, and `accounted_dc_w`).
