...
function requestVideoFullscreenFallback(player, cause) { try { if (player.requestFullscreen) { player.requestFullscreen().catch((error) => showToast(`${ui('无法进入全屏')}：${error.message}`)); return; } } catch (error) { showToast(`${ui('无法进入全屏')}：${error.message}`); return; } showToast(`${ui('无法进入全屏')}：${cause?.message || ui('浏览器不支持全屏播放')}`); }
function requestMediaFullscreen(player) { const target = byId('mediaDialog').querySelector('.media-dialog-shell'); try { if (player.webkitEnterFullscreen) { player.webkitEnterFullscreen(); return; } if (target.requestFullscreen) { target.requestFullscreen().catch((error) => requestVideoFullscreenFallback(player, error)); } else { requestVideoFullscreenFallback(player); } } catch (error) { requestVideoFullscreenFallback(player, error); } }
