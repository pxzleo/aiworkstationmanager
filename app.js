...
async function resetAutomaticTask(task) { const message = window.axisI18n.language === 'zh' ? `将执行中的任务“${task.title}”重新排队？旧执行者将不能回写结果。` : `Requeue running task “${task.title}”? The previous worker will no longer be able to save its result.`; if (!confirm(message)) return; try { await api(`/automatic-tasks/${task.id}/reset`, { method: 'POST' }); showToast(ui('自动任务已重新排队')); await refreshAutomaticTasks(); } catch (error) { showToast(error.message); } }
