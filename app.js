...
function renderUsers() {
  ...
  state.users.forEach((user) => {
    const row = element('div', 'table-row'); const identity = element('span'); const avatar = element('i', 'user-avatar', user.username.slice(0, 1).toUpperCase()); const copy = element('span'); copy.append(element('b', '', user.username), element('small', '', user.is_current ? '当前登录账户' : '管理账户')); identity.append(avatar, copy);
...
async function deleteUser(user) { if (!confirm(`删除用户“${user.username}”？该用户的登录会话将立即失效。`)) return; try { await api(`/users/${user.id}`, { method: 'DELETE' }); showToast('用户已删除'); await refreshUsers(); } catch (error) { showToast(error.message); } }
