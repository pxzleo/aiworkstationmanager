...
function usagePercent(used, total, reported = null) { return finite(used) && finite(total) && total > 0 ? normalizedPercent(used / total * 100) : normalizedPercent(reported); }
function usageTrack(label) { const track = element('div', 'usage-track'); track.setAttribute('role', 'progressbar'); track.setAttribute('aria-valuemin', '0'); track.setAttribute('aria-valuemax', '100'); track.setAttribute('aria-label', ui(label)); track.append(element('i')); return track; }
function setUsageTrack(track, value) { if (!track) return; const amount = normalizedPercent(value); track.style.setProperty('--usage', `${amount ?? 0}%`); track.classList.toggle('unavailable', amount === null); track.classList.toggle('warning', amount !== null && amount >= 90); track.classList.toggle('danger', amount !== null && amount >= 95); if (amount === null) track.removeAttribute('aria-valuenow'); else track.setAttribute('aria-valuenow', String(Math.round(amount))); }
...
async function refreshAll() { await Promise.allSettled([refreshSnapshot(), refreshHistory(), refreshServicesAndScenes(), refreshVideoJobs(), refreshAutomaticTasks(), refreshUsers(), refreshLogs(), refreshSystemInfo(), refreshPowerModel(), refreshFiles(state.filePath)]); }
...
function renderPowerModel() {
  ...
  const calibrated = Boolean(model.calibrated);
  text('powerModelSource', ui(calibrated ? '已用功率计校准' : '默认估算参数'));
  const split = finite(current?.measured_w)
    ? (finite(current?.estimated_w)
      ? `${powerWatts(current.measured_w)} ${ui('实测')} + ${powerWatts(current.estimated_w)} ${ui('估算')}`
      : `${powerWatts(current.measured_w)} ${ui('实测')}`)
    : ui('不支持');
  dataText('powerModelSplit', split, '--');
  dataText('powerModelBaseline', powerWatts(model.baseline_w), '--');
  dataText('powerModelEfficiency', finite(model.psu_efficiency) ? `${(model.psu_efficiency * 100).toFixed(1)}%` : '--', '--');
...
async function submitPowerCalibration(event) {
  ...
  const wall = Number(byId('powerCalibrationWall').value);
  if (!finite(wall) || wall <= 0) return text('powerCalibrationError', ui('请输入功率计读到的墙插功率。'));
  try {
    state.powerModel = await api('/power-model/calibration', { method: 'POST', body: { point: byId('powerCalibrationPoint').value, wall_w: wall } });
    renderPowerModel(); showToast(ui('整机功耗校准已更新')); byId('powerCalibrationWall').value = '';
  } catch (error) { text('powerCalibrationError', error.message); }
}
async function clearPowerCalibration() {
  if (!confirmUi('清除校准后，整机功耗会回到默认估算参数。确定继续？')) return;
  text('powerCalibrationError', '');
  try { state.powerModel = await api('/power-model/calibration', { method: 'DELETE' }); renderPowerModel(); showToast(ui('已清除整机功耗校准')); }
  catch (error) { text('powerCalibrationError', error.message); }
}
...
async function refreshHistory() {
  ...
  try {
    ...
    state.history = (result.samples || []).map(normalizeHistorySample); state.historyBucketSeconds = result.bucket_seconds || 0; state.historyLoaded = true; updateHistoryPeriodNavigation(); renderCharts();
  } catch (error) {
...
function currentSeries() { const samples = state.history.slice(); const current = state.historyFollowingCurrent ? snapshotAsHistory(state.snapshot) : null; if (current && !samples.some((sample) => sample.sampled_at === current.sampled_at)) samples.push(current); return monitorChart.holdPowerReadings(samples, Math.min(120_000, Math.max(30_000, state.historyBucketSeconds * 2_000))); }
...
function renderSnapshot() {
  ...
  text('cpuMetric', percent(cpu.load_percent)); text('cpuDetail', `温度 ${finite(cpu.temperature_c) ? `${Math.round(cpu.temperature_c)}°C` : '不支持'}`); setUsageTrack(byId('cpuUsage'), cpu.load_percent);
  const used = gib(memory.used_bytes); const total = gib(memory.total_bytes); const memoryUsage = usagePercent(memory.used_bytes, memory.total_bytes, memory.percent); text('memoryMetric', used === null ? '不支持' : `${used.toFixed(1)} GB`); text('memoryDetail', total === null ? '总量不可用' : `${percent(memoryUsage)} 已使用 · 共 ${total.toFixed(1)} GB`); setUsageTrack(byId('memoryUsage'), memoryUsage);
  ...
  if (disk) { const diskUsage = usagePercent(disk.used_bytes, disk.total_bytes, disk.percent); text('diskLabel', `${disk.device || disk.mountpoint} 可用`); text('diskMetric', `${gib(disk.total_bytes - disk.used_bytes).toFixed(1)} GB`); text('diskDetail', `${percent(diskUsage)} 已使用`); setUsageTrack(byId('diskUsage'), diskUsage); } else { text('diskMetric', '不支持'); setUsageTrack(byId('diskUsage'), null); }
...
function powerBreakdownText(power) {
  const measured = power?.measured_w; const estimated = power?.estimated_w;
  if (!finite(measured)) return ui('不支持');
  if (!finite(estimated)) return `${Math.round(measured)} W ${ui('实测')}`;
  return `${Math.round(measured)} / ${Math.round(estimated)} W`;
}
function sampleTotalPower(sample) { if (finite(sample?.total_power_w)) return sample.total_power_w; const values = (sample?.gpus || []).map((gpu) => gpu.power_w).filter(finite); return values.length ? values.reduce((sum, value) => sum + value, 0) : null; }
function monitorSummaryItem(label, hint, target, withUsage = false) { const button = element('button', 'monitor-summary-item'); button.type = 'button'; button.dataset.monitorTarget = target; const status = element('i', 'monitor-health-dot'); const copy = element('span'); copy.append(element('small', '', label), userElement('strong', '', '--'), element('em', '', hint)); const bar = withUsage ? usageTrack(label) : null; if (bar) copy.append(bar); button.append(status, copy); return { node: button, value: copy.querySelector('strong'), hint: copy.querySelector('em'), status, bar }; }
...
function buildMonitorSummary(container, details) {
  ...
  const list = element('section', 'monitor-summary-list'); details.summary = new Map(); const items = [{ key: 'CPU', label: 'CPU', hint: '负载与温度', target: 'host', withUsage: true }, { key: '系统内存', label: '系统内存', hint: '物理与提交内存', target: 'host', withUsage: true }, ...state.gpus.map((gpu) => ({ key: gpu._uiKey, label: `GPU ${gpu.index}`, hint: gpu.name || 'NVIDIA GPU', target: 'gpu', gpuKey: gpu._uiKey, withUsage: true })), { key: '存储', label: '存储', hint: '容量、吞吐与延迟', target: 'system', withUsage: true }, { key: '网络', label: '网络', hint: '上传与下载', target: 'system' }, { key: 'WSL / Docker', label: 'WSL / Docker', hint: '运行平台压力', target: 'system' }]; items.forEach(({ key, label, hint, target, gpuKey, withUsage }, index) => { const item = monitorSummaryItem(label, hint, target, withUsage); item.node.style.setProperty('--summary-order', String(index)); item.node.addEventListener('click', () => { if (gpuKey) state.selectedMonitorGpuKey = gpuKey; selectMonitorView(target); }); list.append(item.node); details.summary.set(key, item); }); container.append(list);
}
function buildHostMonitor(container, details, chartIndex) {
  const totalPower = monitorDetail('整机功耗'); const powerBreakdown = monitorDetail('实测 / 估算'); const cpuTemp = monitorDetail('CPU 温度'); const cpuFrequency = monitorDetail('CPU 频率'); const memoryUsage = monitorDetail('内存用量'); const commitUsage = monitorDetail('提交内存'); details.totalPower = totalPower.value; details.powerBreakdown = powerBreakdown.value; details.cpuTemp = cpuTemp.value; details.cpuFrequency = cpuFrequency.value; details.memoryUsage = memoryUsage.value; details.commitUsage = commitUsage.value;
  ...
  const powerPart = (key) => (sample) => sample?.display_power?.[key] ?? monitorChart.powerBreakdown(sample)[key];
  const stackedPower = (...keys) => (sample) => monitorChart.stackedPower(sample, keys);
  const powerSeries = [
    { label: '整机功耗', color: '#f59e0b', getter: sampleTotalPower },
    { label: '3090 功率', color: '#60a5fa', getter: powerPart('gpu3090'), plotGetter: stackedPower('gpu3090') },
    { label: '4090 功率', color: '#a78bfa', getter: powerPart('gpu4090'), plotGetter: stackedPower('gpu3090', 'gpu4090') },
    { label: 'CPU 功率', color: '#22c55e', getter: powerPart('cpu'), plotGetter: stackedPower('gpu3090', 'gpu4090', 'cpu') },
    { label: '其他功率', color: '#f87171', getter: powerPart('other'), valueOnly: true },
  ];
...
function setSummaryStatus(item, value, hint, severity = 'ready', usage = null) { if (!item) return; item.value.textContent = value; item.hint.textContent = hint; item.status.className = `monitor-health-dot ${severity}`; setUsageTrack(item.bar, usage); }
function renderMonitorDetails(samples) {
  ...
  if (details.summary) { let severity = 'ready'; const commitRatio = finite(memory.commit_used_bytes) && finite(memory.commit_limit_bytes) && memory.commit_limit_bytes > 0 ? memory.commit_used_bytes / memory.commit_limit_bytes : null; const lowDisk = (host.disks || []).filter((disk) => usagePercent(disk.used_bytes, disk.total_bytes, disk.percent) !== null).sort((a, b) => usagePercent(b.used_bytes, b.total_bytes, b.percent) - usagePercent(a.used_bytes, a.total_bytes, a.percent))[0]; const lowDiskUsage = lowDisk ? usagePercent(lowDisk.used_bytes, lowDisk.total_bytes, lowDisk.percent) : null; const mark = (next) => { if (next === 'danger' || (next === 'warning' && severity === 'ready')) severity = next; }; setSummaryStatus(details.summary.get('CPU'), percent(host.cpu?.load_percent), finite(host.cpu?.temperature_c) ? `${Math.round(host.cpu.temperature_c)}°C` : ui('温度不支持'), 'ready', host.cpu?.load_percent); setSummaryStatus(details.summary.get('系统内存'), `${gib(memory.used_bytes)?.toFixed(1) ?? '--'} / ${gib(memory.total_bytes)?.toFixed(1) ?? '--'} GB`, commitRatio === null ? ui('提交内存不可用') : `${ui('提交')} ${Math.round(commitRatio * 100)}%`, commitRatio !== null && commitRatio >= .9 ? 'danger' : commitRatio !== null && commitRatio >= .75 ? 'warning' : 'ready', usagePercent(memory.used_bytes, memory.total_bytes, memory.percent)); if (commitRatio >= .75) mark(commitRatio >= .9 ? 'danger' : 'warning'); state.gpus.forEach((gpu) => { const hot = finite(gpu.temperature_c) && gpu.temperature_c >= 85 ? 'danger' : finite(gpu.temperature_c) && gpu.temperature_c >= 75 ? 'warning' : 'ready'; const gpuSeverity = gpu._stale ? 'warning' : hot; const gpuHint = gpu._stale ? `${ui('上次数据')} · ${gpu._lastSuccessAt ? formatDate(gpu._lastSuccessAt, true) : '--'}` : `${gpu.temperature_c ?? '--'}°C · ${gpu.performance_state || '--'}`; setSummaryStatus(details.summary.get(gpu._uiKey), `${percent(gpu.load_percent)} · ${mibToGib(gpu.memory_used_mib)?.toFixed(1) ?? '--'} GB`, gpuHint, gpuSeverity, gpu.load_percent); mark(gpuSeverity); }); const diskSeverity = lowDiskUsage >= 95 ? 'danger' : lowDiskUsage >= 90 ? 'warning' : 'ready'; setSummaryStatus(details.summary.get('存储'), lowDisk ? `${Math.max(0, gib(lowDisk.total_bytes - lowDisk.used_bytes)).toFixed(1)} GB ${ui('可用')}` : '--', lowDisk?.device || ui('未检测到'), diskSeverity, lowDiskUsage); mark(diskSeverity); const network = host.primary_network; setSummaryStatus(details.summary.get('网络'), network ? `${byteRate(network.received_bytes_per_second)} ↓` : '--', network ? `${byteRate(network.sent_bytes_per_second)} ↑ · ${network.name}` : ui('未检测到')); const wsl = host.wsl || {}; const runningContainers = (snapshot.docker?.containers || []).filter((item) => String(item.state).toLowerCase() === 'running').length; setSummaryStatus(details.summary.get('WSL / Docker'), wsl.running ? `${gib(wsl.memory_used_bytes)?.toFixed(1) ?? '--'} GB` : ui('未运行'), `${runningContainers} ${ui('个容器运行')}`, 'ready'); details.health.textContent = severity === 'danger' ? ui('存在异常') : severity === 'warning' ? ui('需要关注') : ui('运行正常'); details.health.className = `monitor-health-label ${severity}`; }
  const currentPower = sampleTotalPower(snapshotAsHistory(snapshot)); if (details.totalPower) details.totalPower.textContent = finite(currentPower) ? `${Math.round(currentPower)} W` : ui('不支持'); if (details.powerBreakdown) details.powerBreakdown.textContent = powerBreakdownText(host.power); if (details.cpuTemp) details.cpuTemp.textContent = finite(host.cpu?.temperature_c) ? `${Math.round(host.cpu.temperature_c)}°C` : ui('不支持'); if (details.cpuFrequency) details.cpuFrequency.textContent = finite(host.cpu?.frequency_mhz) ? `${Math.round(host.cpu.frequency_mhz)} MHz` : ui('不支持'); const used = gib(memory.used_bytes); const total = gib(memory.total_bytes); if (details.memoryUsage) details.memoryUsage.textContent = used !== null && total !== null ? `${used.toFixed(1)} / ${total.toFixed(1)} GB` : ui('不支持'); if (details.commitUsage) details.commitUsage.textContent = finite(memory.commit_used_bytes) && finite(memory.commit_limit_bytes) ? `${gib(memory.commit_used_bytes).toFixed(1)} / ${gib(memory.commit_limit_bytes).toFixed(1)} GB` : ui('不支持');
  ...
  if (details.storageVolumes) {
    details.storageVolumes.replaceChildren();
    const volumes = (host.disks || []).filter((disk) => finite(disk.total_bytes));
    if (!volumes.length) details.storageVolumes.append(element('p', 'empty-state', '存储容量不可用。'));
    else volumes.forEach((disk) => {
      const used = gib(disk.used_bytes); const total = gib(disk.total_bytes); const free = total !== null && used !== null ? Math.max(0, total - used) : null;
      const capacity = usagePercent(disk.used_bytes, disk.total_bytes, disk.percent);
      const row = element('div', 'storage-volume-row'); const heading = element('div', 'storage-volume-heading');
      heading.append(userElement('strong', '', disk.device || disk.mountpoint || '--'), element('b', '', `${percent(capacity)} ${ui('已使用')}`), element('span', '', `${used?.toFixed(1) ?? '--'} / ${total?.toFixed(1) ?? '--'} GB`), element('small', '', `${free?.toFixed(1) ?? '--'} GB ${ui('可用')}`));
      const bar = usageTrack(`${disk.device || disk.mountpoint || ''} ${ui('已使用')}`); setUsageTrack(bar, capacity);
      row.append(heading, bar); details.storageVolumes.append(row);
    });
  }
...
function scenesForDisplay() { return [...state.scenes.filter((scene) => scene.state === 'active'), ...state.scenes.filter((scene) => scene.state !== 'active')]; }
function moveSceneCard(sceneId, targetId, after = false) {
  ...
  if (displayedScenes.find((scene) => scene.id === sceneId)?.state === 'active') return;
  if (displayedScenes.find((scene) => scene.id === targetId)?.state === 'active') after = true;
  const ids = displayedScenes.map((scene) => scene.id).filter((id) => id !== sceneId);
...
function moveSceneByOffset(sceneId, offset) {
  const displayedScenes = scenesForDisplay(); const ids = displayedScenes.map((scene) => scene.id); const index = ids.indexOf(sceneId); const next = index + offset;
  if (index < 0 || next < 0 || next >= ids.length || displayedScenes[index].state === 'active' || displayedScenes[next].state === 'active') return;
...
function renderOverviewSceneSelect() {
  ...
  if (select.dataset.sceneSignature !== signature) {
    ...
    scenesForDisplay().forEach((scene) => { const option = userElement('option', '', `${scene.name}${scene.state === 'active' ? ui('（当前）') : ''}`); option.value = scene.id; select.append(option); });
...
function renderScenes() {
  ...
  displayedScenes.forEach((scene, index) => {
    ...
    const up = iconButton('上移场景', 'arrow-up'); up.disabled = index === 0 || displayedScenes[index - 1]?.state === 'active' || scene.busy || actionGuard.pending; up.addEventListener('click', () => moveSceneByOffset(scene.id, -1));
    const down = iconButton('下移场景', 'arrow-down'); down.disabled = scene.state === 'active' || index === displayedScenes.length - 1 || scene.busy || actionGuard.pending; down.addEventListener('click', () => moveSceneByOffset(scene.id, 1));
