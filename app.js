...
function buildMonitorCharts() {
  const container = document.querySelector('.monitor-grid'); container.replaceChildren(); state.chartSpecs = []; state.correlationControllers = []; const details = { gpus: new Map() }; state.monitorDetails = details; const overview = element('div', 'monitor-overview'); const coverage = monitorDetail('已累计'); const sampleCount = monitorDetail('图表点'); const lastSample = monitorDetail('最近更新'); overview.append(coverage.node, sampleCount.node, lastSample.node); details.coverage = coverage.value; details.sampleCount = sampleCount.value; details.lastSample = lastSample.value; container.append(overview); let chartIndex = 0;
...
function renderMonitorDetails(samples) {
  const details = state.monitorDetails; if (!details) return; const snapshot = state.snapshot || {}; const host = snapshot.host || {}; const memory = host.memory || {}; details.coverage.textContent = historyCoverageLabel(); details.sampleCount.textContent = String(samples.length); details.lastSample.textContent = samples.length ? formatDate(samples.at(-1).sampled_at) : '--';
