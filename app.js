...
function videoJobStageLabel(job) { if (job.status === 'running' && job.progress?.state === 'queued') return window.axisI18n.language === 'zh' ? `ComfyUI 排队 ${job.progress.queue_position || ''}`.trim() : `ComfyUI queue ${job.progress.queue_position || ''}`.trim(); const label = { queued: '等待调度', waiting_for_ninfer_idle: '等待 NInfer 空闲', waiting_for_memory: '等待内存释放', switching_to_video: '切换生成场景', checking_comfy: '检查 ComfyUI', submitting: '提交工作流', running: 'ComfyUI 生成中', collecting_output: '收集输出', restoring_scene: '恢复原场景', restoring_code: '恢复原场景', verifying_ninfer: '恢复原场景', callback_pending: '回调 OpenCode', succeeded: '已完成', failed: '失败', cancelled: '已取消' }[job.status]; return label ? ui(label) : job.status; }
function videoJobProgress(job) { if (['succeeded', 'failed', 'cancelled'].includes(job.status)) return 100; if (job.status === 'running') return job.progress?.state === 'queued' ? 45 : 55; return { queued: 0, waiting_for_ninfer_idle: 8, waiting_for_memory: 12, switching_to_video: 25, checking_comfy: 35, submitting: 40, collecting_output: 85, restoring_scene: 90, restoring_code: 92, verifying_ninfer: 95, callback_pending: 98 }[job.status] ?? 0; }
...
function renderVideoJobs() { const list = byId('videoJobList'); if (!list) return; list.replaceChildren(); const queued = Number(state.videoQueueSummary?.queued_segments) || 0; const nonterminal = Number(state.videoQueueSummary?.nonterminal_segments) || 0; text('videoJobNavCount', String(nonterminal)); text('videoQueuedSegments', window.axisI18n.language === 'zh' ? `等待处理：${queued} 段` : `Waiting: ${queued} segments`); if (!state.videoJobs.length) { list.append(element('p', 'empty-state', '尚无视频任务。')); return; } state.videoJobs.forEach((job) => { const failed = job.status === 'failed'; const row = element('article', `operation-row video-job-row${failed ? ' operation-failed' : ''}`); const copy = element('div'); const segment = job.batch_size > 1 ? (window.axisI18n.language === 'zh' ? ` · 第 ${job.batch_index} / ${job.batch_size} 段` : ` · Segment ${job.batch_index} / ${job.batch_size}`) : ''; const title = userElement('strong', '', `${window.axisI18n.language === 'zh' ? '任务' : 'Job'} ${job.id.slice(0, 8)}${segment} · ${window.axisI18n.language === 'zh' ? '会话' : 'Session'} ${job.session_id}`); const specs = element('div', 'video-job-specs'); videoJobSpecLabel(job).forEach((value) => specs.append(element('span', '', value))); const details = element('div', 'operation-summary'); const activity = job.progress && Number.isFinite(job.progress.processing) ? `NInfer processing ${job.progress.processing} · deferred ${job.progress.deferred} · slots ${(job.progress.active_slots || []).join(',') || '-'}` : ''; const detail = job.error_summary || job.output_path || activity || job.workflow_path; const route = `${ui('生成场景')} ${job.generation_scene_name || '-'} · ${ui('原场景')} ${job.original_scene_name || ui('等待记录')}`; details.append(userElement('small', '', `${route} · ${detail}`), element('span', 'operation-actor', job.prompt_id ? `prompt_id · ${job.prompt_id}` : '尚未提交 ComfyUI')); const progress = videoJobProgress(job); const progressBox = element('div', 'video-job-progress'); const progressHead = element('span'); progressHead.append(element('small', '', window.axisI18n.language === 'zh' ? '阶段进度' : 'Stage progress'), element('b', '', `${progress}%`)); const progressTrack = element('i'); progressTrack.append(element('b')); progressTrack.firstChild.style.width = `${progress}%`; progressBox.append(progressHead, progressTrack); copy.append(title, specs, details, progressBox); const badge = element('span', `status-label ${job.status === 'succeeded' ? 'ready' : failed || job.status === 'cancelled' ? 'danger' : 'partial'}`, videoJobStageLabel(job)); row.append(copy, badge, element('time', '', formatDate(job.updated_at, true))); if (!['succeeded', 'failed', 'cancelled'].includes(job.status)) { const cancel = labeledIconButton('取消任务', 'x', 'button secondary video-job-cancel'); cancel.addEventListener('click', () => cancelVideoJob(job)); row.append(cancel); } list.append(row); }); }
async function cancelVideoJob(job) { if (!confirmUi(`取消视频任务 ${job.id.slice(0, 8)}？若已在 ComfyUI 执行，将请求中断并恢复原场景。`)) return; try { await api(`/video-jobs/${job.id}/cancel`, { method: 'POST' }); showToast('视频任务取消请求已提交'); await refreshVideoJobs(); } catch (error) { showToast(error.message); } }
