...
function normalizeHistorySample(sample) { return { ...sample, total_power_w: finite(sample.total_power_w) ? Number(sample.total_power_w) : null, measured_power_w: finite(sample.measured_power_w) ? Number(sample.measured_power_w) : null, estimated_power_w: finite(sample.estimated_power_w) ? Number(sample.estimated_power_w) : null, cpu_load_percent: normalizedPercent(sample.cpu_load_percent), memory_percent: normalizedPercent(sample.memory_percent), gpus: Array.isArray(sample.gpus) ? sample.gpus.map(normalizeGpu) : [], disks: Array.isArray(sample.disks) ? sample.disks : [] }; }
