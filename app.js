...
const state = {
  ...
  history: [], services: [], scenes: [], users: [], operations: [], videoJobs: [], timers: new Map(),
...
async function refreshVideoJobs() { if (document.hidden) return; try { const result = await api('/video-jobs?limit=100', { resource: 'video-jobs' }); state.videoJobs = result.jobs || []; renderVideoJobs(); } catch (error) { if (!(error instanceof StaleRequestError)) showToast(`视频任务读取失败：${error.message}`); } }
...
function videoJobStageLabel(job) { if (job.status === 'running' && job.progress?.state === 'queued') return window.axisI18n.language === 'zh' ? `ComfyUI 排队 ${job.progress.queue_position || ''}`.trim() : `ComfyUI queue ${job.progress.queue_position || ''}`.trim(); const label = { queued: '等待调度', waiting_for_ninfer_idle: '等待 NInfer 空闲', switching_to_video: '切换 Video Gen', checking_comfy: '检查 ComfyUI', submitting: '提交工作流', running: 'ComfyUI 生成中', collecting_output: '收集输出', restoring_code: '恢复 Code Agent', verifying_ninfer: '验证 NInfer', callback_pending: '回调 OpenCode', succeeded: '已完成', failed: '失败', cancelled: '已取消' }[job.status]; return label ? ui(label) : job.status; }
function renderVideoJobs() { const list = byId('videoJobList'); if (!list) return; list.replaceChildren(); const active = state.videoJobs.filter((job) => !['succeeded', 'failed', 'cancelled'].includes(job.status)); text('videoJobNavCount', String(active.length)); if (!state.videoJobs.length) { list.append(element('p', 'empty-state', '尚无视频任务。')); return; } state.videoJobs.forEach((job) => { const failed = job.status === 'failed'; const row = element('article', `operation-row video-job-row${failed ? ' operation-failed' : ''}`); const copy = element('div'); const title = userElement('strong', '', `任务 ${job.id.slice(0, 8)} · 会话 ${job.session_id}`); const details = element('div', 'operation-summary'); const activity = job.progress && Number.isFinite(job.progress.processing) ? `NInfer processing ${job.progress.processing} · deferred ${job.progress.deferred} · slots ${(job.progress.active_slots || []).join(',') || '-'}` : ''; const detail = job.error_summary || job.output_path || activity || job.workflow_path; details.append(userElement('small', '', detail), element('span', 'operation-actor', job.prompt_id ? `prompt_id · ${job.prompt_id}` : '尚未提交 ComfyUI')); copy.append(title, details); const badge = element('span', `status-label ${job.status === 'succeeded' ? 'ready' : failed || job.status === 'cancelled' ? 'danger' : 'partial'}`, videoJobStageLabel(job)); row.append(copy, badge, element('time', '', formatDate(job.updated_at, true))); if (!['succeeded', 'failed', 'cancelled'].includes(job.status)) { const cancel = labeledIconButton('取消任务', 'x', 'button secondary video-job-cancel'); cancel.addEventListener('click', () => cancelVideoJob(job)); row.append(cancel); } list.append(row); }); }
async function cancelVideoJob(job) { if (!confirmUi(`取消视频任务 ${job.id.slice(0, 8)}？若已在 ComfyUI 执行，将请求中断并恢复 Code Agent。`)) return; try { await api(`/video-jobs/${job.id}/cancel`, { method: 'POST' }); showToast('视频任务取消请求已提交'); await refreshVideoJobs(); } catch (error) { showToast(error.message); } }
