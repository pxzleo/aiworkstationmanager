...
function renderOverviewSceneSelect() {
  const select = byId('overviewSceneSelect'); const signature = JSON.stringify(state.scenes.map((scene) => [scene.id, scene.name, scene.state]));
  if (select.dataset.sceneSignature !== signature) {
    ...
    state.scenes.forEach((scene) => { const option = element('option', '', `${scene.name}${scene.state === 'active' ? '（当前）' : ''}`); option.value = scene.id; select.append(option); });
...
function renderScenes() {
  text('sceneNavCount', String(state.scenes.length)); renderOverviewSceneSelect(); const list = byId('sceneList'); list.replaceChildren(); if (!state.scenes.length) { list.append(element('p', 'empty-state', '尚未添加场景。')); text('activeSceneName', '尚未添加场景'); renderOperationTimeline(); return; }
  state.scenes.forEach((scene, index) => {
    ...
    const top = element('div', 'scene-panel-top'); const topActions = element('span', 'scene-panel-meta'); const handle = element('button', 'scene-drag-handle', '⠿'); handle.type = 'button'; handle.title = '拖动调整场景位置'; handle.setAttribute('aria-label', `拖动调整 ${scene.name} 的位置`); topActions.append(element('i', scene.state === 'active' ? 'scene-status' : '', scene.state === 'active' ? '已激活' : '部分完成'), handle); top.append(element('span', '', `场景 ${String(index + 1).padStart(2, '0')}`), topActions);
    panel.append(top, element('h2', '', scene.name), element('p', '', scene.description || '无说明'));
    ...
    sceneServices.forEach((service, order) => { const item = element('div'); item.append(element('span', '', `启动顺序 ${order + 1}`), element('strong', '', service.name)); const meta = element('small', 'scene-service-meta'); const status = element('i', 'scene-service-status'); status.append(element('i', `service-state ${statusClass(service.status.state)}`), document.createTextNode(service.busy ? '操作中' : statusLabels[service.status.state] || '未知')); meta.append(status); item.append(meta); if (service.ui_url) { const ui = element('button', 'scene-ui-link', '打开 UI ↗'); ui.type = 'button'; ui.addEventListener('click', () => window.open(service.ui_url, '_blank', 'noopener,noreferrer')); item.append(ui); } map.append(item); });
  ...
  const active = state.scenes.find((item) => item.state === 'active'); text('activeSceneName', active ? `${active.name} · 已激活` : '场景未完整激活'); text('activeSceneSummary', active ? active.description || `包含 ${active.service_ids.length} 个服务` : '当前服务状态不完整符合任何场景。');
...
function renderSceneProgress(scene, operation) {
  ...
  steps.forEach((step) => { const item = element('li', step.status); item.append(element('time', '', formatDate(step.started_at)), element('span', '', `${step.action === 'start' ? '启动' : '停止'} ${targetName('service', step.target_id)}`), element('em', '', step.status === 'running' ? '进行中' : step.status === 'succeeded' ? '成功' : step.status === 'interrupted' ? '已终止' : '失败')); log.append(item); });
