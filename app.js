...
function buildHostMonitor(container, details, chartIndex) {
  ...
  const stackedPower = (...keys) => (sample) => { const power = monitorChart.powerBreakdown(sample); if (!finite(power.other)) return null; const sum = keys.reduce((total, key) => total + power[key], 0); return sum <= sample.total_power_w ? sum : null; };
  const powerSeries = [
