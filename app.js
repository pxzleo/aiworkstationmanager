...
function renderScenes() {
  ...
  state.scenes.forEach((scene, index) => {
    ...
    const actions = element('div', 'scene-card-actions'); const reorder = element('span', 'scene-reorder-controls'); const up = element('button', 'button secondary', '上移'); up.disabled = index === 0 || scene.busy || actionGuard.pending; up.addEventListener('click', () => moveSceneByOffset(scene.id, -1)); const down = element('button', 'button secondary', '下移'); down.disabled = index === state.scenes.length - 1 || scene.busy || actionGuard.pending; down.addEventListener('click', () => moveSceneByOffset(scene.id, 1)); reorder.append(up, down); const details = element('button', 'button secondary', '详细说明'); details.addEventListener('click', () => openSceneDetails(scene)); const activate = element('button', 'button primary', scene.state === 'active' ? '重新切换' : '切换到此场景'); activate.disabled = scene.busy || actionGuard.pending; activate.addEventListener('click', () => activateScene(scene)); const defaultButton = element('button', 'button secondary', scene.is_default ? '取消默认' : '设为默认'); defaultButton.disabled = scene.busy || actionGuard.pending; defaultButton.addEventListener('click', () => setDefaultScene(scene)); const edit = element('button', 'button secondary', '编辑'); edit.disabled = scene.busy || actionGuard.pending; edit.addEventListener('click', () => openSceneDialog(scene)); const remove = element('button', 'button secondary', '删除'); remove.disabled = scene.busy || actionGuard.pending; remove.addEventListener('click', () => deleteScene(scene)); actions.append(reorder, details, activate, defaultButton, edit, remove); panel.append(map, actions); list.append(panel);
  });
