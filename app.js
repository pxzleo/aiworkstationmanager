...
function snapshotAsHistory(snapshot) { return snapshot ? { sampled_at: snapshot.sampled_at, cpu_load_percent: snapshot.host?.cpu?.load_percent, cpu_temperature_c: snapshot.host?.cpu?.temperature_c, memory_percent: snapshot.host?.memory?.percent, memory_used_bytes: snapshot.host?.memory?.used_bytes, memory_total_bytes: snapshot.host?.memory?.total_bytes, gpus: snapshot.gpus || [] } : null; }
...
function buildMonitorCharts() {
  ...
  const hostMemoryCapacity = (samples) => { const capacities = [gib(state.snapshot?.host?.memory?.total_bytes), ...(samples || []).map((sample) => gib(sample.memory_total_bytes))].filter((value) => finite(value) && value > 0); return capacities.length ? Math.max(...capacities) : 1; };
  const hostSpecs = [{ kicker: 'CPU', title: '处理器负载', description: '全部逻辑处理器综合使用率', color: 'var(--accent)', getter: (sample) => sample.cpu_load_percent }, { kicker: 'RAM', title: '系统内存', description: '已用内存与物理内存容量', color: '#60a5fa', getter: (sample) => gib(sample.memory_used_bytes), unit: 'GB', decimals: 1, maximum: hostMemoryCapacity, initialMaximum: hostMemoryCapacity([]), showMaximumInCurrent: true, statisticsIncludeUnit: true }];
  ...
  state.gpus.forEach((gpu, position) => {
    ...
    const memoryChart = createMonitorChart({ kicker: 'VRAM', title: '显存占用', description: '已用显存与物理显存容量', color, getter: metricGib('memory_used_mib'), unit: 'GB', decimals: 1, maximum: memoryCapacity, initialMaximum: memoryCapacity([]), showMaximumInCurrent: true, statisticsIncludeUnit: true, compact: true }, chartIndex++); state.chartSpecs.push(memoryChart);
