...
function renderScenes() {
  ...
  state.scenes.forEach((scene, index) => {
    ...
    if (scene.is_default || scene.state === 'active') { const banners = element('div', 'scene-state-banners'); if (scene.is_default) { const banner = element('div', 'scene-default-banner'); banner.append(element('strong', '', '默认启动场景'), element('span', '', 'AXIS 启动时自动切换')); banners.append(banner); } if (scene.state === 'active') { const banner = element('div', 'scene-active-banner'); banner.append(element('strong', '', '当前已激活场景'), element('span', '', '服务组合正在生效')); banners.append(banner); } panel.append(banners); }
