...
class H3WorkflowBuilderTests(unittest.TestCase):
    def test_automatic_task_skill_claims_finishes_and_repeats_serially(self) -> None:
        ...
        self.assertIn("不得等待人工介入", skill)
        self.assertIn("不会改变授权范围、任务目标、关键产物或造成不可逆影响", skill)
        self.assertIn("现有项目约定、默认值和最小可行方案", skill)
        self.assertIn("安全的只读检查", skill)
        self.assertIn("不得因偏好性细节缺失而失败", skill)
        self.assertIn("不可替代的凭据、素材", skill)
        self.assertIn("任务内容内部矛盾", skill)
        self.assertIn("关键目标或关键产物规格", skill)
