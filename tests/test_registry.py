...
class DatabaseRegistryTests(unittest.TestCase):
    def test_electricity_rate_survives_database_reopen(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "manager.db"
            database = Database(path)
            self.assertEqual(database.read_electricity_rate(), 0.5)
            self.assertEqual(database.write_electricity_rate(0.67, "admin"), 0.67)
            self.assertEqual(Database(path).read_electricity_rate(), 0.67)
    ...
    def test_automatic_task_claim_is_atomic_across_sessions(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            database = Database(Path(temporary) / "manager.db")
            database.create_automatic_task("a" * 32, "并发领取测试", "admin", "127.0.0.1")
            barrier = threading.Barrier(2)

            def claim(session_id: str) -> tuple[str, dict[str, Any] | None]:
                barrier.wait()
                return database.claim_automatic_task(session_id)

            with ThreadPoolExecutor(max_workers=2) as executor:
                results = list(executor.map(claim, ("session-a", "session-b")))

            self.assertEqual(sorted(result[0] for result in results), ["busy", "claimed"])
    ...
    def test_schema_seventeen_persists_prunes_and_aggregates_resource_history(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["samples"][0]["cpu_power_w"], 15)
            self.assertEqual(result["samples"][0]["power_sample_count"], 2)
            self.assertEqual(result["samples"][0]["cpu_power_sample_count"], 2)
            self.assertEqual(result["samples"][0]["gpus"][0]["power_sample_count"], 2)
