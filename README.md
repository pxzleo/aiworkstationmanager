...
当前工作站已按用户明确要求安装 `AXIS-AI-Workstation-Manager` 开机启动计划任务。任务使用当前部署目录、本机专用配置和正式服务数据库，只启动管理器本身，不自动启动任何已登记服务。
...
5. 不批量调用 `status`；目标服务保存状态全为 `running` 且没有非目标服务仍为 `running` 时，场景为“已激活”。非空场景内没有任何目标服务保存为 `running` 时显示“未激活”；已有部分目标服务启动但尚未完整匹配场景时显示“部分启动”。空场景在没有任何服务运行时为“已激活”，存在运行中服务时为“部分启动”。
