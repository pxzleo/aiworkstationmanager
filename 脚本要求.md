...
管理器默认每 5 秒并行调用所有空闲服务的 `status`。同一个服务的状态检查和启动、停止、重启动作会串行执行，不会由管理器同时调用。
...
- 动作返回后，管理器会立即再次调用 `status` 验证最终状态：
  - `start` 和 `restart` 必须最终得到 `running`。
  - `stop` 必须最终得到 `stopped`。
- 即使动作退出码为 `0`，最终状态不符合预期时，管理器仍会将操作记为失败。
