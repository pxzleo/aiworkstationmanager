...
| GET | `/api/v1/automatic-tasks/models` | 已登录用户实时读取本机 OpenCode 可选模型 |
...
| POST | `/api/v1/automatic-tasks/execution/start-local` | 仅限本机 OpenCode 使用插件请求 AXIS 启动整个队列，不读取浏览器 Cookie |
| POST | `/api/v1/automatic-tasks` | 已登录管理员使用 CSRF 新增任务，提交 `content` 和可选 `model` |
...
| POST | `/api/v1/automatic-tasks/{id}/reset` | 已登录管理员使用 CSRF 把执行中、已完成或失败任务重新排队；清除已有结果和执行所有权，保留累计执行次数并使旧领取令牌失效 |
| POST | `/api/v1/automatic-tasks/claim` | 仅限本机 OpenCode，使用当前 `session_id` 按已保存队列顺序原子领取首个 `pending` 任务；AXIS 启动的单项会话还须提交 `expected_task_id`，顺序变化时拒绝领取 |
...
执行页保存整个队列共用的父目录和每日本机时间，定时开关默认关闭；立即执行与开关无关。AXIS 按顺序为每条任务在父目录下新建独立子目录，并通过本机 OpenCode CLI 新建单项会话，要求其使用 `axis-automatic-tasks` Skill 只领取指定任务，回写后退出再启动下一条，每日触发日期写入数据库防止重启后重复执行。启用时若时间已过，当天不补跑。启动状态与错误显示在执行页；单项任务状态仍以数据库中的队列记录为准。

页面输入任务内容并可选择执行模型，标题由首个有效句子自动压缩生成，并通过不随任务状态变化的创建顺序分页完整加载后在页面按执行顺序展示。未执行任务可通过上移、下移按钮调整顺序；保存时服务端原子核对页面读取的旧顺序和全部 `pending` 任务，发现并发变化时明确拒绝，OpenCode 严格按成功保存的顺序领取。执行中、已完成或失败任务都可重新排队并追加到当前队尾；已有结果和执行所有权会清除，累计执行次数保留并在再次领取时增加。执行中任务的旧令牌立即失效，但已经启动的外部操作不会被取消。Skill 领取任务后不再询问用户或等待人工介入；不会改变授权范围、任务目标、关键产物且不造成不可逆影响的非关键选择使用现有项目约定、默认值和最小可行方案，并记录到结果摘要；可通过工作区、配置或安全只读检查确定的信息必须自行确定。只有不可替代的凭据、素材或外部条件缺失、有限重试后仍不可恢复、任务内容内部矛盾、关键目标或产物规格无法可靠确定、客观验收标准仍不能满足，或需要新的高影响授权时才将任务明确标记失败。“无人介入”不扩大删除、发布、付款和外部消息等授权。`axis-automatic-tasks` Skill 在普通会话中调用插件请求 AXIS 启动队列，在 AXIS 新建的单项会话中调用指定任务的领取、续期和完成工具；插件领取后每分钟后台续期 30 分钟租约，长时间工具调用、单次响应错误以及会话空闲等待 AXIS 视频生成与回调期间也持续续期。`session.idle`、`status=idle` 和单次 `session.error` 都不结束自动任务租约。网络和服务端瞬时错误继续重试；任务不存在、租约过期或所有权不匹配等明确拒绝会停止后台续期。过期领取在下一次领取时自动回收，旧令牌不能回写。一条任务回写状态且对应会话退出后，AXIS 才在新的子目录和新会话启动下一项；单项失败会记录已完成检查和原因并继续。运行中的任务不能从页面编辑或删除，但可人工重新排队；另一 OpenCode 会话不能并发领取。运行 `integrations/opencode/Install-AxisAutomaticTasks.ps1` 可把插件和 Skill 安装到当前用户配置目录，重启 OpenCode 后即可通过“启动自动任务”触发。
...
- `/api/v1/history` 的 `window` 使用分钟格式，默认 `15m`，范围为 `1m..44700m`；可选的带时区 ISO 8601 `end` 指定查询结束时刻，查询只返回 `[end-window, end)` 内的采样。`15m` 返回原始采样，`1h` 按 15 秒、自然日按 60 秒、自然周按 15 分钟、自然月按 1 小时分桶。响应除 `samples` 外仍返回 `bucket_seconds`、`retention_minutes`、`stored_sample_count`、`stored_since` 和 `stored_until`，供客户端判断历史数据覆盖范围；当前资源监控界面不显示这些元数据。
