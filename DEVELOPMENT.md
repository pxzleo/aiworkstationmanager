...
- `/api/v1/history` 的 `window` 使用分钟格式，默认 `15m`，范围为 `1m..1440m`；响应为 `{"window":"15m","samples":[...]}`。
...
默认数据库是 `data/workstation-manager.db`，当前 schema 为 13，并在启动时自动迁移。同一个数据库同一时间只允许一个管理器实例使用，避免重复执行服务脚本。

服务的保存状态是控制面的主要状态来源。资源监控定时采样不会调用服务脚本；只有显式状态检查才执行 `status`。
