...
默认数据库是 `data/workstation-manager.db`，当前 schema 为 14，并在启动时自动迁移。同一个数据库同一时间只允许一个管理器实例使用，避免重复执行服务脚本。

服务的保存状态是控制面的主要状态来源。资源监控定时采样不会调用服务脚本；只有显式状态检查才执行 `status`。资源采样将 CPU、内存及每张 GPU 的精简指标写入 SQLite，默认保留 24 小时；内存队列固定只保留最近 15 分钟。
