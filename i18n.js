...
(function initializeI18n() {
  ...
  const translations = {
    ...
    '自动任务页面': 'Automatic task pages', '任务列表': 'Task list', '执行': 'Execution',
    '使用下面的工作目录启动 OpenCode，并按队列顺序执行全部未执行任务。': 'Start OpenCode in the directory below and run all pending tasks in queue order.',
    '工作目录': 'Working directory', '每天执行时间（本机时间）': 'Daily time (local)',
    '工作目录（父目录）': 'Working directory (parent)',
    '这里填写统一的父目录。每条任务会在其中创建独立子目录，并启动新的 OpenCode 会话按顺序执行。': 'Enter a shared parent directory. Each task gets its own child directory and new OpenCode session, run in queue order.',
    '当前任务目录': 'Current task directory',
    '启用每天定时执行': 'Enable daily execution', '保存设置': 'Save settings',
    '立即执行未执行队列': 'Run pending queue now', '正在读取执行设置。': 'Loading execution settings.',
    '尚未从本页面启动': 'Not started from this page', 'OpenCode 正在执行': 'OpenCode is running',
    '上次启动已完成': 'Last run completed', '上次启动失败': 'Last run failed',
    '状态未知': 'Unknown status', '定时启动': 'Scheduled start', '手动启动': 'Manual start',
    ...
    '显示风格保存在当前浏览器，电价保存在 AXIS 中。': 'The display style is saved in this browser; the electricity rate is saved in AXIS.',
    '电费设置': 'Electricity rate', '整机功耗曲线的窗口电费按窗口耗电量乘以每度电价格计算。': 'Window cost on the total power chart is window energy multiplied by the electricity rate.',
    '每度电价格（元/度）': 'Rate (CNY/kWh)', '保存电价': 'Save rate', '请输入有效的每度电价格。': 'Enter a valid electricity rate.', '电价已保存': 'Electricity rate saved',
    ...
    '持久采样 · 最近 3 个月': 'Persistent samples · Last 3 months', '历史范围': 'History range',
    '选择历史时间': 'Select history time', '上一周期': 'Previous period', '下一周期': 'Next period',
    '最近 1 小时': 'Last hour', '1周': '1 week', '1月': '1 month',
    ...
    '实测传感器加上主板、内存、供电与电源损耗的估算': 'Measured sensors plus an estimate for the motherboard, memory, VRM and PSU losses',
    '实测 / 估算': 'Measured / estimated', '实测': 'measured', '估算': 'estimated',
    '3090 功率': '3090 power', '4090 功率': '4090 power', 'CPU 功率': 'CPU power', '其他功率': 'Other power',
    'RTX 3090 板卡实测功耗': 'Measured RTX 3090 board power', 'RTX 4090 板卡实测功耗': 'Measured RTX 4090 board power',
    'CPU 封装实测功耗': 'Measured CPU package power', '其他传感器及主板、供电、电源估算': 'Other sensors and motherboard, VRM and PSU estimates',
    '功率曲线 · 同步时间': 'Power curves · synchronized time',
    '整机功耗校准': 'Total system power calibration',
    '传感器只能读到 GPU 板卡和 CPU 封装功耗，主板、内存、硬盘、风扇、CPU 供电损耗和电源转换损耗都读不到，只能估算。用功率计读一次墙插功率就能把估算校准成实测。': 'Sensors only expose GPU board power and CPU package power. The motherboard, memory, drives, fans, CPU VRM losses and PSU conversion losses cannot be read and are estimated. One wall-meter reading calibrates that estimate against reality.',
    '清除校准': 'Clear calibration', '当前口径': 'Current basis',
    '固定底噪': 'Fixed baseline', '电源效率': 'PSU efficiency',
    '已用功率计校准': 'Calibrated with a power meter', '默认估算参数': 'Default estimate parameters',
    '校准点': 'Calibration point', '空闲': 'Idle', '满载': 'Full load',
    '墙插功率（W）': 'Wall power (W)', '按当前负载校准': 'Calibrate at current load',
    '记录的是提交这一刻的实测功率，请在负载稳定后再提交。空闲和满载两个点都提交后会同时解出底噪和电源效率；只提交一个点时，电源效率沿用配置默认值。': 'The measured power recorded is the value at the moment you submit, so let the load settle first. Submitting both the idle and full-load points solves for the baseline and the PSU efficiency together; with only one point the PSU efficiency keeps its configured default.',
    '请输入功率计读到的墙插功率。': 'Enter the wall power reported by your power meter.',
    '整机功耗校准已更新': 'Total system power calibration updated',
    '已清除整机功耗校准': 'Total system power calibration cleared',
    '清除校准后，整机功耗会回到默认估算参数。确定继续？': 'Clearing the calibration returns total system power to the default estimate parameters. Continue?',
