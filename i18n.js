...
(function initializeI18n() {
  ...
  const translations = {
    ...
    '用户管理': 'User Management', '日志中心': 'Operation Log', '视频任务': 'Video Jobs', '自动任务': 'Automatic Tasks', '文件服务': 'File Service',
    ...
    '打开': 'Open', '播放': 'Play', '下载': 'Download', '这个目录是空的。': 'This folder is empty.',
    '配置的根目录不存在或不可访问。': 'The configured root folder does not exist or cannot be accessed.',
    '根目录未配置': 'Root folder is not configured', '媒体播放': 'Media player', '下载文件': 'Download File',
    '排序': 'Sort', '文件排序方式': 'File sort order', '时间：最新优先': 'Time: newest first', '时间：最早优先': 'Time: oldest first',
    '名称：升序': 'Name: A to Z', '名称：降序': 'Name: Z to A', '大小：从大到小': 'Size: largest first', '大小：从小到大': 'Size: smallest first',
    '文件查看方式': 'File view', '列表': 'List', '缩略图': 'Thumbnails', '无法进入全屏': 'Unable to enter fullscreen', '无法退出全屏': 'Unable to exit fullscreen',
    ...
    'OpenCode 串行队列': 'OpenCode Serial Queue', '新增任务': 'Add Task',
    '在这里准备任务内容并调整未执行任务的顺序；启动后将按当前顺序逐项执行。': 'Prepare tasks and arrange pending work. Once started, tasks run one at a time in the displayed order.',
    '任务标题会根据内容自动生成。执行中的任务不能编辑或删除；单项失败会记录原因并继续下一项。': 'Titles are generated from task content. Running tasks cannot be edited or deleted; a failed item records its cause and the queue continues.',
    '尚无自动任务。': 'No automatic tasks.', 'OpenCode 自动任务': 'OpenCode Automatic Task',
    ...
    '完成时间': 'Finished', '开始时间': 'Started',
    '执行中的任务不能编辑': 'Running tasks cannot be edited', '执行中的任务不能删除': 'Running tasks cannot be deleted',
    '自动任务读取失败': 'Unable to load automatic tasks', '请输入任务内容。': 'Enter task instructions.',
    '自动任务已更新并设为未执行': 'Automatic task updated and reset to pending', '自动任务已添加': 'Automatic task added',
    '自动任务已删除': 'Automatic task deleted', '重新排队': 'Requeue', '自动任务已重新排队': 'Automatic task requeued',
    '上移任务': 'Move task up', '下移任务': 'Move task down', '任务顺序已保存': 'Task order saved',
    '生成场景': 'Generation scene', '原场景': 'Original scene', '等待记录': 'Pending capture',
    '等待调度': 'Waiting for scheduler', '等待 OpenCode 当前响应结束': 'Waiting for the current OpenCode response to finish', '等待 NInfer 空闲': 'Waiting for NInfer idle',
    '等待内存释放': 'Waiting for memory release',
    'AXIS 会等待开发服务空闲后切换到生成场景；批次逐段处理，完成后自动恢复原场景并通知 OpenCode。': 'AXIS waits for development services to become idle before switching scenes. Batch segments run one at a time; AXIS restores the original scene and notifies OpenCode when complete.',
    '切换生成场景': 'Switching generation scene', '检查 ComfyUI': 'Checking ComfyUI',
    '提交工作流': 'Submitting workflow', 'ComfyUI 生成中': 'Generating in ComfyUI',
    '收集输出': 'Collecting output', '复制到共享目录': 'Copying to shared folder', '恢复原场景': 'Restoring original scene',
    '回调 OpenCode': 'Calling back OpenCode',
    '已完成': 'Completed', '失败': 'Failed', '已取消': 'Cancelled',
    ...
    '正在载入 AXIS': 'Loading AXIS', '正在检查管理器': 'Checking Manager', '正在确认首次设置与登录状态。': 'Checking setup and sign-in status.',
    ...
    '由管理器维护 WSL 局域网映射': 'Let AXIS manage the WSL LAN mapping',
    'WSL 发行版': 'WSL distribution', 'Windows 监听地址': 'Windows listen address',
    'Windows 监听端口': 'Windows listen port', 'WSL 目标端口': 'WSL target port',
    '默认使用服务端口': 'Defaults to the service port', '默认同监听端口': 'Defaults to the listen port',
    '例如 RTX 4090': 'For example, RTX 4090', '取消': 'Cancel', '保存服务': 'Save Service',
    '场景编辑器': 'Scene Editor', '场景名称': 'Scene name', '简短介绍': 'Short Introduction',
    '详细使用说明': 'Detailed Usage Instructions', '场景详细说明': 'Scene Details',
    '默认生成场景': 'Default generation scene',
    'OpenCode 未指定生成场景名称时使用此场景。': 'Use this scene when OpenCode does not specify a generation scene name.',
    '详细说明': 'Details', '暂无详细说明。': 'No detailed instructions.', '关闭': 'Close',
    '上移场景': 'Move scene up', '下移场景': 'Move scene down',
    '查看详细说明': 'View scene details', '编辑场景': 'Edit scene', '删除场景': 'Delete scene',
    '设为默认场景': 'Set as default scene', '取消默认场景': 'Clear default scene',
    '上移服务': 'Move service up', '下移服务': 'Move service down',
    '填写外部 URL、协议、模型名和调用注意事项': 'Enter the external URL, protocol, model name, and usage notes',
    '需要启动的服务': 'Services to start',
    ...
    '场景切换进度': 'Scene Switch Progress', '操作进度': 'Operation Progress', '正在切换场景': 'Switching Scene',
    ...
    '意外停止': 'Unexpectedly stopped', '外部启动': 'Started externally', '映射异常': 'Port mapping error',
    ...
    '管理器正在确认需要停止的服务并按顺序执行。': 'The manager is identifying services to stop, then stopping them in order.', '正在确认需要停止的服务': 'Identifying services to stop',
    '无需执行服务步骤，正在确认最终状态。': 'No service steps are required. Confirming the final state.',
    '返回服务列表': 'Return to Services', '全部服务已停止': 'All services stopped', '停止操作已终止': 'Stop operation cancelled',
    '停止全部服务失败': 'Failed to stop all services', '停止全部服务未完成': 'Stopping all services did not complete', '所有已登记服务均已停止。': 'All registered services have stopped.',
  ...
  const templates = [
    ...
    [/^删除自动任务“(.+)”？$/, 'Delete automatic task “$1”?'],
    [/^未执行：(.+) 项$/, '$1 pending'], [/^执行中：(.+) · 未执行：(.+)$/, '$1 running · $2 pending'],
  ];
